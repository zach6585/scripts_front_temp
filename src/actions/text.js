...
export const getTexts = (script_number_and_mentee_id) => dispatch => {
    ...
    axios.get('http://localhost:3001/texts')
      .then(res => dispatch({ type: TEXTS_SUCCESS, payload: res.data.filter(text_data => text_data.script === script_number_and_mentee_id.script_number.toString() && text_data.mentee_id === script_number_and_mentee_id.mentee_id)}))
